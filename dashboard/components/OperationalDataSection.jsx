...
const OperationalDataSection = ({ tabela }) => {
    ...
    const columns = useMemo(() => {
        ...
        const keys = Object.keys(row).filter(k => COLUMN_LABELS[k]);
        const labels = keys.map(k => COLUMN_LABELS[k] ?? k);
    ...
    const rowsFormatted = useMemo(() => {
        return visibleRows.map((row) => {
            ...
            for (const k in row) {
                if (k === "item_status") {
                    out[COLUMN_LABELS[k] ?? k] = normalizeStatus(row[k]);
                } else {
                    out[COLUMN_LABELS[k] ?? k] = row[k];
                }
            }
            return out;
        });
    ...
    return (
        <div className="operational-section-wrapper">
            ...
            <Row className="mb-0">
                <Col xs={12}>
                    ...
                    <div className="operational-table-container">
                        ...
                        >
                            <DataTable columns={columns} rows={rowsFormatted} premium={true} />
                        </div>
                    </div>
            ...
            <ModalComponent
                ...
                content={<DataTable columns={columns} rows={modalRows} premium={true} />}
            />
        </div>
    );
};
